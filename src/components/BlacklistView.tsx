...
interface BlacklistViewProps {
  ...
  onRemoveFromBlacklist: (id: number) => void;
  onBatchRemoveFromBlacklist: (ids: number[]) => void;
...
export const BlacklistView: React.FC<BlacklistViewProps> = ({
  ...
  const blacklistItems = ledgers.filter(item => {
  ...
  const totalPages = Math.ceil(blacklistItems.length / pageSize) || 1;
  const pagedItems = blacklistItems.slice((currentPage - 1) * pageSize, currentPage * pageSize);
  ...
  const triggerRemoveOne = (item: LedgerItem) => {
    setModalConfig({
      ...
      content: (
        <div>
          <p>确定要将台账 <strong className="text-gray-900 font-semibold">【{item.name}】</strong> 移出黑名单吗？</p>
          <p className="mt-1.5 text-gray-500 text-xs">移出后该台账将恢复并重新显示在{ledgerTypeName}列表中。</p>
        </div>
      ),
      onConfirm: () => {
        onRemoveFromBlacklist(item.id);
        closeModal();
      }
    });
  };

  const triggerAddToWhitelistOne = (item: LedgerItem) => {
    setModalConfig({
      isOpen: true,
      title: '确认加入白名单',
      confirmType: 'primary',
      ...
      content: (
        <div>
          <p>确定要将台账 <strong className="text-gray-900 font-semibold">【{item.name}】</strong> 直接移入白名单吗？</p>
          <p className="mt-1.5 text-gray-500 text-xs">加入白名单后，该台账将被赋予重点关注属性，集中在白名单列表中管控。</p>
        </div>
      ),
      onConfirm: () => {
        if (onAddToWhitelist) {
  ...
  const handleBatchRemove = () => {
    if (selectedIds.length === 0) {
      setModalConfig({
        ...
        confirmType: 'primary',
        confirmText: '我知道了',
        content: <span>请先勾选需要移出黑名单的台账。</span>,
    ...
    setModalConfig({
      ...
      content: (
        <div>
          <p>确定要将选中的 <strong className="text-blue-600 font-semibold">{selectedIds.length}</strong> 项台账移出黑名单并恢复至{ledgerTypeName}列表吗？</p>
        </div>
      ),
      onConfirm: () => {
        onBatchRemoveFromBlacklist(selectedIds);
        setSelectedIds([]);
        closeModal();
      }
    });
  ...
  const handleBatchAddToWhitelist = () => {
    if (selectedIds.length === 0) {
      setModalConfig({
        ...
        confirmType: 'primary',
        confirmText: '我知道了',
        content: <span>请先勾选需要加入白名单的台账。</span>,
        onConfirm: closeModal
      });
      return;
    }
    setModalConfig({
      ...
      content: (
        <div>
          <p>确定要将选中的 <strong className="text-green-600 font-semibold">{selectedIds.length}</strong> 项台账直接移入白名单吗？</p>
        </div>
      ),
      onConfirm: () => {
        if (onBatchAddToWhitelist) {
          ...
          selectedIds.forEach(id => onAddToWhitelist(id));
        }
        setSelectedIds([]);
        closeModal();
      }
  ...
  return (
    <div className="bg-white flex flex-col space-y-4">
      ...
      {/* Header Info Banner */}
      <div className="flex items-center justify-between bg-[#fff1f0] border border-[#ffa39e] px-4 py-2.5 rounded-sm text-xs text-gray-700">
        <div className="flex items-center space-x-2">
          <i className="fa-solid fa-ban text-[#ff4d4f] text-sm"></i>
          <span>
            加入黑名单的台账已从{ledgerTypeName}列表中移除并在本页集中管控，当前共有 <strong className="text-[#ff4d4f] font-semibold">{blacklistItems.length}</strong> 条黑名单台账。
            （注意：仅可在黑名单页面中将台账「移出黑名单」恢复）
          </span>
        </div>
        {onNavigateToTab && (
          <button
      ...
      {/* Filter Bar */}
      <LedgerFilterBar
        totalCount={blacklistItems.length}
      ...
      <div className="flex items-center justify-between bg-[#e6f4ff] border border-[#91caff] px-3.5 py-2 rounded-sm text-xs text-gray-700">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="bg-white border border-[#1677ff] text-[#1677ff] px-3 py-1 rounded-sm text-xs hover:bg-blue-50 transition-colors cursor-pointer flex items-center"
          >
            <i className="fa-solid fa-arrow-rotate-left mr-1.5 text-[11px]"></i> 批量移出黑名单
          </button>
          <button
        ...
        <table className="w-full text-left text-xs table-fixed">
          <thead className="bg-[#fafafa] text-gray-700 border-b border-[#e8e8e8]">
            <tr>
              ...
              <th className="px-2 py-2.5 font-normal w-[22%] whitespace-nowrap">URL地址/简介</th>
              <th className="px-1.5 py-2.5 font-normal w-[110px] whitespace-nowrap">认证/研判</th>
              ...
              pagedItems.map((item, idx) => {
                ...
                return (
                    ...
                    <td className="px-2 py-2 overflow-hidden">
                      <div className="flex items-center space-x-1.5 min-w-0">
                        <img
                          src={item.avatar}
                          alt={item.name}
                          className="w-6 h-6 rounded-full object-cover border border-gray-200 flex-shrink-0 grayscale opacity-80"
                        />
                  ...
                  <td className="px-1 py-2 whitespace-nowrap text-center overflow-hidden">
                    <span className="px-1 py-0.2 bg-gray-100 text-gray-600 text-[10px] rounded-xs border border-gray-200 whitespace-nowrap truncate inline-block" title={item.source}>
                      {item.source}
                    </span>
                  </td>
                  <td className="px-1 py-2 whitespace-nowrap text-center overflow-hidden">
                    <span className="px-1 py-0.2 bg-red-50 text-[#ff4d4f] text-[10px] rounded-xs border border-red-200 font-medium whitespace-nowrap inline-block">
                      黑名单
                    </span>
                  </td>
                  <td className="px-1.5 py-2 text-center whitespace-nowrap overflow-hidden">
                    <div className="flex items-center justify-center space-x-2 text-xs">
                      <button
                        ...
                        className="text-[#1677ff] hover:underline cursor-pointer bg-transparent border-none p-0 text-xs font-normal whitespace-nowrap"
                        title="移出黑名单"
                      >
                        移出黑名单
                      </button>
                      <button
                        onClick={() => triggerAddToWhitelistOne(item)}
              ...
              <tr>
                <td colSpan={10} className="py-12 text-center text-gray-400">
                  <div className="flex flex-col items-center justify-center space-y-1.5">
                    ...
                    <p className="text-xs">暂无黑名单台账数据</p>
                  </div>
                </td>
              </tr>
            )}
      ...
      <div className="flex items-center justify-between text-xs text-gray-600 px-1 py-1">
        <span>共 {blacklistItems.length} 条数据，每页 {pageSize} 条</span>
